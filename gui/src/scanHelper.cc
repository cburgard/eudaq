    ...
    while(i<1000) {
        if(scanConf->HasSection(std::to_string(i))) {
            ...
            if(name == "wrongPara" || param == "wrongPara"
               || (std::numeric_limits<double>::epsilon()>std::abs(start - std::numeric_limits<double>::lowest()))
               || (std::numeric_limits<double>::epsilon()>std::abs(stop - std::numeric_limits<double>::lowest()))
               || (std::numeric_limits<double>::epsilon()>std::abs(step - std::numeric_limits<double>::lowest())))
